<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8 mt10">
      <el-col :span="1.5">
        <el-button v-hasPerm="['admin','sys:dept:save']" type="primary" icon="el-icon-plus" size="small" @click="openDialog(undefined, '添加部门', 'Add', true)">
          添加
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button v-hasPerm="['admin','sys:dept:update']" type="success" icon="el-icon-edit" size="small" :disabled="single" @click="openDialog(ids[0], '修改部门', 'Edit', true)">
          修改
        </el-button>
      </el-col>
    </el-row>
    ...
    <el-table row-key="id" :tree-props="{children: 'children'}" stripe :header-cell-style="{background:'#eef1f6',color:'#606266'}" v-loading="loading" :data="tableData" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="contacts" label="联系人手机号">
        <template slot-scope="scope">
          {{ scope.row.contacts }}
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        align="center">
        <template slot-scope="scope">
          <el-button v-hasPerm="['admin','sys:dept:get']" type="text" size="small" icon="el-icon-view" @click.native.stop="openDialog(scope.row.id, '查看部门', 'Details', false)">查看</el-button>
          <el-button v-hasPerm="['admin','sys:dept:save']" type="text" size="small" icon="el-icon-plus" @click.native.stop="openDialog(scope.row.id, '添加子部门', 'Add', true)">添加子部门</el-button>
          <el-button v-hasPerm="['admin','sys:dept:update']" type="text" size="small" icon="el-icon-edit" @click.native.stop="openDialog(scope.row.id, '修改部门', 'Edit', true)">修改</el-button>
          <el-button v-hasPerm="['admin','sys:dept:remove']" type="text" size="small" icon="el-icon-delete" @click.native.stop="handleRemove(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--end========表格列表===========end-->
