<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8 mt10">
      <el-col :span="1.5">
        <el-button v-hasPerm="['admin','sys:notice:save']" type="primary" icon="el-icon-plus" size="small" @click="openDialog(undefined, '添加通知公告', 'Add', true)">
          添加
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button v-hasPerm="['admin','sys:notice:update']" type="success" icon="el-icon-edit" size="small" :disabled="single" @click="openDialog(ids[0], '修改通知公告', 'Edit', true)">
          修改
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button v-hasPerm="['admin','sys:notice:remove']" type="danger" icon="el-icon-delete" size="small" :disabled="multiple" @click="handleRemove">
          删除
        </el-button>
      </el-col>
    </el-row>
    ...
    <el-table stripe :header-cell-style="{background:'#eef1f6',color:'#606266'}" v-loading="loading" :data="tableData" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="type" label="公告类型">
        <template slot-scope="scope">
          <m-dict mode="list" v-model="scope.row.type" dict-key="sys_notice_type"></m-dict>
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        align="center">
        <template slot-scope="scope">
          <el-button v-hasPerm="['admin','sys:notice:get']" type="text" size="small" icon="el-icon-view" @click.native.stop="openDialog(scope.row.id, '查看通知公告', 'Details', false)">查看</el-button>
          <el-button v-hasPerm="['admin','sys:notice:update']" type="text" size="small" icon="el-icon-edit" @click.native.stop="openDialog(scope.row.id, '修改通知公告', 'Edit', true)">修改</el-button>
          <el-button v-hasPerm="['admin','sys:notice:remove']" type="text" size="small" icon="el-icon-delete" @click.native.stop="handleRemove(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--end========表格列表===========end-->
